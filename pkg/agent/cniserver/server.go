...
func ParseConf(request *cnipb.CniRequest) (*types.NetConf, *CNIArgs, error) {
	// parse request Stdin
	conf := &types.NetConf{}
...
func ParseResult(result *types100.Result) (*cnipb.CniResponse, error) {
	var resultBytes bytes.Buffer
	if err := result.PrintTo(&resultBytes); err != nil {
		return nil, err
	}
...
func (s *CNIServer) CmdAdd(ctx context.Context, request *cnipb.CniRequest) (*cnipb.CniResponse, error) {
	...
	conf, args, err := ParseConf(request)
	if err != nil {
	...
	result.IPs[0].Interface = types100.Int(0)
...
func (s *CNIServer) CmdDel(ctx context.Context, request *cnipb.CniRequest) (*cnipb.CniResponse, error) {
	...
	return ParseResult(&types100.Result{
		CNIVersion: conf.CNIVersion,
	})
}
