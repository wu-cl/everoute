...
var _ = Describe("PolicyController", func() {
	...
	Context("policy needed endpoints and groups has been create", func() {
		...
		When("create a sample policy with port range", func() {
			...
			BeforeEach(func() {
				policy = newTestPolicy(group1.Name, group2.Name, group3.Name, newTestPort("TCP", "1000-1999"), newTestPort("UDP", "65535"))
			...
			It("should flatten policy to rules", func() {
				...
				assertHasPolicyRuleWithPortRange(ctx, policy, "Egress", "Allow", "192.168.2.1/32",
					0, 0, "192.168.1.1/32", 0xffff, 0, "UDP")
